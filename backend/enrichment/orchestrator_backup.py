...
async def generate_rows(
    ...
    async with NetrowsClient(api_key=netrows_key) as nc:
        # Primary fetch
        primary_payload = await _safe_call(_call_endpoint(nc, primary, query, row_limit), primary, events, progress)
        ...
        person_sources = [s for s in sources if s == "linkedin_people" and s != primary]
        for ps in person_sources[:1]:
            extra = await _safe_call(_call_endpoint(nc, ps, query, row_limit), ps, events, progress)
...
async def _call_endpoint(nc: NetrowsClient, source: str, query: str, limit: int):
    if source == "linkedin_people":
        return await nc.linkedin_people(query, limit=limit)
    if source == "linkedin_companies":
        return await nc.linkedin_companies(query, limit=limit)
    ...
    if source == "linkedin_jobs":
        return await nc.linkedin_jobs(query, limit=limit)
    if source == "indeed_jobs":
        return await nc.indeed_jobs(query, limit=limit)
    # Crunchbase is enricher-only (BETA, no search) — handled in dedicated phase
    if source == "yc_search":
        return await nc.yc_search(query=query, per_page=max(limit, 20))
...
def _fallback_cell(h: str, it: dict, domain_emails: dict[str, list[dict]]) -> Any:
    ...
    # Direct key hits — use sentinel to distinguish 'missing' from 'False/0'
    for k in (nh, nh.replace(" ", "_"), nh.replace(" ", "")):
    if k in it and it[k] not in (None, ""):
        return it[k]
    v = None
    # ---- fallback chain (each branch may set v) ----
    ...
    elif nh in ("year founded",): v = it.get("year_founded") or it.get("founded")
    ...
    elif nh in ("founded", "founded year", "year founded"): v = it.get("founded")
    elif nh in ("logo",): v = it.get("logo")
    elif nh in ("description",) and it.get("tagline"): v = it.get("description") or it.get("tagline")
    ...
    elif nh in ("school", "education", "university"):
        v = it.get("school")
    elif nh in ("country",):
        v = it.get("country") or (it.get("geo") or {}).get("country") if isinstance(it.get("geo"), dict) else it.get("country")
    elif nh in ("city",):
        v = it.get("city") or (it.get("geo") or {}).get("city") if isinstance(it.get("geo"), dict) else it.get("city")
    elif nh in ("skills",):
        sk = it.get("skills") or []
        if sk and isinstance(sk[0], str):
            v = ", ".join(sk[:8])
        else:
            v = ", ".join(s.get("name") for s in sk[:8] if isinstance(s, dict) and s.get("name")) or None
    elif nh in ("job id", "jobid"): v = it.get("jobId") or it.get("id")
    elif nh in ("salary", "compensation", "pay"): v = it.get("salaryRange") or (it.get("salary") if isinstance(it.get("salary"), str) else None)
    ...
    elif nh in ("posted", "posted at", "date posted"): v = it.get("postedAt")
    ...
    if not v and it.get("githubLogin"):
            v = f"https://github.com/{it['githubLogin']}"
    ...
    elif nh in ("one liner", "tagline", "headline"): v = it.get("one_liner") or it.get("tagline") or it.get("headline")
    elif nh in ("yc url", "ycombinator url"): v = it.get("ycombinator_url") or (f"https://www.ycombinator.com/companies/{it.get('slug')}" if it.get("slug") else None)
    ...
    # ---- Crunchbase person mappings ----
        ...
        rs = it.get("regions")
        v = ", ".join(rs) if isinstance(rs, list) else rs
    elif nh in ("tags",):
        ts = it.get("tags")
        v = ", ".join(ts) if isinstance(ts, list) else ts
    elif nh in ("founder", "founders", "founder name"):
        v = it.get("founderNames") or it.get("founderName")
    ...
    elif nh in ("categories",): v = it.get("cbCategories") or (", ".join(it.get("categories")) if isinstance(it.get("categories"), list) else it.get("categories"))
    # ---- Indeed mappings ----
    elif nh in ("job key",): v = it.get("job_key")
    elif nh in ("snippet",): v = it.get("snippet")
    elif nh in ("is remote",): v = it.get("is_remote")
    elif nh in ("indeed url",): v = it.get("indeed_url")
    elif nh in ("city",): v = it.get("city")
    elif nh in ("state",): v = it.get("state")
    elif nh in ("salary min",): v = it.get("salary_min")
    elif nh in ("salary max",): v = it.get("salary_max")
    elif nh in ("benefits",): v = ", ".join(it.get("benefits") or []) if isinstance(it.get("benefits"), list) else it.get("benefits")
    ...
    if em:
    v = em[0].get("email") or em[0].get("value")
    return v
...
def _backfill_nulls(rows: list[dict], items: list[dict], headers: list[str],
    ...
    by re-deriving from the matching raw item. The LLM occasionally drops
...
async def _llm_normalize(
    ...
    system = (
        ...
        "If a field is unknown for a row, set it to null. Match emails to companies by domain. "
    ...
    for r in (raw_rows or [])[:row_limit]:
        ...
        for h in headers:
            v = r.get(h)
            if v is None:
                v = r.get(normalize_header(h))
            src = "netrows" if v else "ai"
            conf = "medium" if v else "low"
